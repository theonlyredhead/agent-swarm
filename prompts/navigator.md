...
```
{
  "relevant_files": ["path/to/file.js", "path/to/other.js"],
  "root_cause_summary": "Precise, technical description of the root cause. Include the function name, line range if known, what is missing or wrong, and what the correct behaviour should be. Be specific enough that the Coder can act without reading the failure context again.",
...
## Rules
- `relevant_files` must contain ONLY files that need to be read or edited to fix this issue — not the entire repo
- `root_cause_summary` must explain WHY the bug exists, not just WHAT fails
- `confidence` below 0.6 means you need more context — say so in root_cause_summary
- Never include test files, lock files, or generated files in relevant_files unless the bug is in a test
